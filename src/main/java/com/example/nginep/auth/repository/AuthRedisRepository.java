...
public class AuthRedisRepository {
    ...
    public void saveJwtKey(String email, String jwtKey) {
        valueOps.set(STRING_KEY_PREFIX + email, jwtKey, 24, TimeUnit.HOURS);
    }
    ...
    public String getVerificationKey(String email) {
        return valueOps.get(STRING_VERIFICATION_KEY_PREFIX + email);
    }

    public void deleteVerificationKey(String email) {
        valueOps.getOperations().delete(STRING_VERIFICATION_KEY_PREFIX + email);
    }

    public void saveVerificationKey(String email, String verificationKey) {
        valueOps.set(STRING_VERIFICATION_KEY_PREFIX + email, verificationKey, 1, TimeUnit.HOURS);
    }
}
